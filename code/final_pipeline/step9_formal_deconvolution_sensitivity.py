...
def prepare_fractions():
    music = pd.read_csv(
        "data/processed/r_music_weighted_fractions.csv",
        index_col=0,
    )
    ...
    bayesprism = pd.read_csv(
        "data/processed/r_bayesprism_theta.csv",
        index_col=0,
    )
    bayesprism["patient_id"] = bayesprism.index.map(patient_of)
    bayesprism = bayesprism[
        bayesprism.index.str.contains("-01A-", regex=False)
    ]
    bayesprism = bayesprism.groupby("patient_id").mean(numeric_only=True)
    ...
    music_summary = pd.DataFrame(
        {
            ...
            "music_immune_fraction": music[
                ["Myeloid", "T_cell", "NK_cell", "B_plasma"]
            ].sum(axis=1),
            "music_stromal_fraction": music[
                ["Fibroblast", "Myofibroblast", "Endothelial"]
            ].sum(axis=1),
        }
    )

    bayesprism_summary = pd.DataFrame(
        {
            "bp_normal_urothelial_fraction": bayesprism["Normal_urothelial"],
            "bp_tumor_epithelial_fraction": bayesprism["Tumor_epithelial"],
            "bp_immune_fraction": bayesprism[
                ["Myeloid", "T_cell", "NK_cell", "B_plasma", "Mast_cell"]
            ].sum(axis=1),
            "bp_stromal_fraction": bayesprism[
                ["Fibroblast", "Myofibroblast", "Endothelial"]
            ].sum(axis=1),
        }
    )
    return music_summary, bayesprism_summary
...
def main():
    ...
    scores = pd.read_csv(
        "data/processed/signature_scores_tcga.csv.gz",
        compression="gzip",
    )
    ...
    base = clinical.join(
        music_fractions.join(bayesprism_fractions, how="inner"),
        how="inner",
    )
    ...
    model_definitions = {
        "music_formal": [
            ...
            "sex_female",
            "stage_num",
            "music_tumor_epithelial_fraction",
            "music_normal_urothelial_fraction",
            "music_immune_fraction",
            "music_stromal_fraction",
        ],
        "bayesprism_formal": [
            "age",
            "sex_female",
            "stage_num",
            "bp_tumor_epithelial_fraction",
            "bp_normal_urothelial_fraction",
            "bp_immune_fraction",
            "bp_stromal_fraction",
        ],
    }
    ...
    sensitivity.to_csv(
        processed / "signature_sensitivity_formal_deconvolution.csv",
        index=False,
    )

    print("primary significant signatures tested:", len(primary_significant))
    print("model rows:", len(sensitivity))
    for model_name, group in sensitivity.groupby("model"):
        print(
            ...
            group["pmid"].nunique(),
            "significant",
            int((group["pvalue"] < 0.05).sum()),
            "failed",
            int(group["pvalue"].isna().sum()),
        )
